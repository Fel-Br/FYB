...
float calc_pulse_req(float L, float DPPR, float diameter) {
  float pulse_req = L*2*PI*DPPR/(PI*diameter);
  return pulse_req;
}
...
float calc_len_x(float calc_x, float calc_theta, float Qx, float Qy, float Ax){
  float len_x = calc_x + cos(theta)*Qx - sin(theta)*Qy - Ax;
  return len_x;
}
// Função para o calculo do tamanho do cabo em y
float calc_len_y(float calc_y,  float calc_theta, float Qx, float Qy, float Ay){
  float len_y = calc_y + sin(theta)*Qx + cos(theta)*Qy - Ay;
  return len_y;
}
...
void loop() {
  ...
  if (newData == true) {
        ...
        if(strstr(input_action,motor_calibration)>0){
          float direction = input_x; // 1 for forward, -1 for backward;
          float c_motor = input_y; // which motor to move;
          float time_count = input_theta; //time estimated to run the motor;
          if(c_motor == 1){
            if(direction == 1){
              Motor1.Forward();
              delay(time_count);
        ...
        if(strstr(input_action,execute)> 0 || (strstr(input_action,calibrate) > 0) ){
          x_goal = input_x;
          y_goal = input_y;
          theta_goal = input_theta*71/4068.0;
  ...
  if (validation_motor_1 == 1 && validation_motor_2 == 1 && validation_motor_3 == 1 && validation_motor_4 == 1){
    ...
    // Pulsos requeridos por cada motor
    ...
    // Pulsos realizados por cada motor
    Serial.print("Enc_1:");
  ...
  if ((x != x_goal) or (y != y_goal) or (theta != theta_goal)){
    ...
    len_goal_1 = sqrt(pow(calc_len_x(x_goal, theta_goal, Qx_1, Qy_1, Ax_1),2) + pow(calc_len_y(y_goal, theta_goal, Qx_1, Qy_1, Ay_1),2));
    len_goal_2 = sqrt(pow(calc_len_x(x_goal, theta_goal, Qx_2, Qy_2, Ax_2),2) + pow(calc_len_y(y_goal, theta_goal, Qx_2, Qy_2, Ay_2),2));
    len_goal_3 = sqrt(pow(calc_len_x(x_goal, theta_goal, Qx_3, Qy_3, Ax_3),2) + pow(calc_len_y(y_goal, theta_goal, Qx_3, Qy_3, Ay_3),2));
    len_goal_4 = sqrt(pow(calc_len_x(x_goal, theta_goal, Qx_4, Qy_4, Ax_4),2) + pow(calc_len_y(y_goal, theta_goal, Qx_4, Qy_4, Ay_4),2));
    ...
    pulse_req_1 = calc_pulse_req(Delta_len_1,DPPR_1, diameter);
    pulse_req_2 = calc_pulse_req(Delta_len_2,DPPR_2, diameter);
    pulse_req_3 = calc_pulse_req(Delta_len_3,DPPR_3, diameter);
    pulse_req_4 = calc_pulse_req(Delta_len_4,DPPR_4, diameter);
